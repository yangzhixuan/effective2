...
There are three families of generators:
  ...
  * Scoped operations, i.e. operations of the form
    ...
    A_1 -> ... A_n -> Prog sig x -> ... -> Prog sig x -> Prog sig x
    @
    (without the requirement of algebraicity law), are generated using
...
and it (together with its signature functors, patterns, ...) can be generated by
@
...
--------------------------------------------------------------------------------
-- makeGen: generate Alg-style effect boilerplate from a base functor
--------------------------------------------------------------------------------
...
makeAlg sigQ =
  do sig <- sigQ
     (opName, ty) <- case sig of
       SigE (UnboundVarE opName) ty -> pure (opName, ty)
       SigE (VarE        opName) ty -> pure (opName, ty)
       _ -> fail "makeAlg: the argument should be of the form \"op :: ...\" for some name op"
     let (tvBndrs, ctx, ty') =
            case ty of
              ForallT tvs ctx ty' -> (tvs, ctx, ty');
              _ -> ([], [], ty)
     when (not (null ctx)) $
       fail "makeAlg: constraints on type arguments are not yet supported"
     kName <- newName "k"
     let (argTys, resTy) = splitArrs ty'
     arity <-
       case resTy of
         LitT (NumTyLit n) -> pure (fromIntegral n)
         _ -> fail "makeAlg: the last type in the given signature should just be a number literal indicating the arity, like \"op :: ... -> 2\""
...
-- | Generate the full bundle of an algebraic operation from base signature functor.
...
makeAlgPiecesFromDec baseName dec = do
  (tvbs, conName, conFields) <- case dec of
    (DataD    [] _ tvbs _ [con] _) -> pure (tvbs, conNameOf con, conArgTys con)
    (NewtypeD [] _ tvbs _  con  _) -> pure (tvbs, conNameOf con, conArgTys con)
  ...
  let allTvNames = map tvbName tvbs
  ...
  let effTy   = applyType (ConT effName) (map VarT tvNames)
      ...
      -- opTy    = funType (paramFields ++ replicate arity progRes) progRes
  ...
  let makeFun :: Name -> Type -> Clause -> FunPieces
      ...
      smartDef = Clause (map VarP argNames)
                        (NormalB (joinExp (VarE 'call `AppE` opExp)))
                        []
