...
open  eff@(Effn n op)
  | n == fromInteger (fromSNat (natSing @(Length effs))) = Right (unsafeCoerce @(_ f a) @(eff f a) op)
...
openEff (Effn n op)
  ...
  | otherwise = Nothing
  where n' = fromInteger (fromSNat (natSing @(EffIndex eff effs)))
...
openEffs effn@(Effn n op)
  ...
  where
    m = fromInteger (fromSNat (natSing @(Length effs)))
...
inj = Effn n
  where
    n = fromInteger (fromSNat (natSing @(EffIndex eff effs)))
